...
const setEditQuotation = (props) => {
  if (props.updateQuotation && props.updateQuotation.selectUpdate) {
    ...
    props.preQuotation &&
    props.preQuotation.selectUpdate &&
    props.preQuotation.selectUpdate.products.length
  ) {
    return {
...
export const FormQuotation = (props) => {
  ...
  const [selectUpdate, setSelectUpdate] = useState(
    updateQuotationAux ? updateQuotationAux.selectUpdate : ""
  );
  const [clientSelectUpdate, setCientSelectUpdate] = useState(
    updateQuotationAux ? updateQuotationAux.selectUpdate.client : null
  );
  const [userSelectUpdate, setUserSelectUpdate] = useState(
    updateQuotationAux ? updateQuotationAux.selectUpdate.user : null
  );

  const [idSelectUpdate, setIdSelectUpdate] = useState(
    updateQuotationAux ? updateQuotationAux.selectUpdate.id : null
  );
    ...
    for (let index in data.products) {
      if (!data.products[index].prices) {
        setOpenAlert({
          open: true,
          message:
            "Antes de finalizar debe editar los precios por unidad de los productos pendientes.",
          type: "error",
        });
    ...
    <div className="container-body body--create-Quotaion">
      <div className="title m-0">
        <div className="title-actions">
          {redirectList && (
            <Redirect to={{ pathname: "/cotizaciones", state: openAlert }} />
          )}
          {openEmail && (
            <SendEmail cancelEmail={cancelEmail} sendEmail={confEmail} />
          )}
      ...
      <div className="quotation-body">
        <form id="quotationForm" className="form-quotation">
          ...
          <TextField
            ...
            label="Cliente"
            name="client"
            onChange={(e) => setCientSelectUpdate(e.target.value)}
          ...
          <TextField
            ...
            name="user"
            onChange={(e) => setUserSelectUpdate(e.target.value)}
      ...
      <div className="col-12 px-0 d-flex justify-content-end container-button">
        <Button
          ...
          onClick={() =>
            idSelectUpdate
              ? updateQuotation("En progreso")
              : saveQuotation("En progreso")
          }
        >
          Guardar cotización
        </Button>
      ...
      <Snackbar
        ...
        onClose={() => setOpenAlert({ alert: { open: false } })}
        anchorOrigin={{
          vertical: "bottom",
          horizontal: "left",
        }}
